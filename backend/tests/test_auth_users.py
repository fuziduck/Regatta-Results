...
class TestPerUserLogin:
    ...
    def test_club_officer_admin_accounts_login(self, test_club):
        """The club's officer/admin email accounts authenticate with their own
        passcodes, under the role the account was given."""
        for role, pin in (("officer", TEST_OFFICER_PIN), ("admin", TEST_ADMIN_PIN)):
            body = _login_user(role, club_user_username(role, test_club["id"]), pin,
                               test_club["id"])
            assert body["role"] == role
            assert body["club_id"] == test_club["id"]
            assert body["username"] == club_user_username(role, test_club["id"])
...
class TestUserCrudScoping:
    ...
    def test_short_passcode_rejected(self, test_club, club_admin_token):
        r = requests.post(f"{API}/users", json={
            "club_id": test_club["id"], "role": "officer",
            "username": "shorty@test.club", "passcode": "12"}, headers=h(club_admin_token))
        assert r.status_code == 400

    def test_admin_cannot_delete_own_account(self, test_club):
        me = _login_user("admin", club_user_username("admin", test_club["id"]),
                         TEST_ADMIN_PIN, test_club["id"])
