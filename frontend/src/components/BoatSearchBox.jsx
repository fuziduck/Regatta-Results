...
// The site search: typing 2+ characters runs the unified search (clubs,
// classes, series, boats) live and drops down grouped matches, each linking
// to its page. Type tabs filter the results. Keyboard: ↑/↓ move the
// highlight, Enter opens it (the first result when nothing is highlighted),
// Escape closes then clears; "/" or Cmd/Ctrl+K focuses the search from
// anywhere on the page.
//
// variant="hero" (default) embeds the open field in a landing hero;
// variant="header" is a search icon in the top bar that opens the same field
// over the page, and closes again on Escape or a click away.
export default function BoatSearchBox({ className = "mt-5", variant = "hero" }) {
  ...
  // Close the dropdown — and in the header the field itself — when clicking
  // anywhere outside the box.
  useEffect(() => {
    const onDocClick = (e) => {
      if (boxRef.current && !boxRef.current.contains(e.target)) {
        setOpen(false);
        if (inHeader) setExpanded(false);
      }
    };
  ...
  // Opening the header field puts the cursor straight in it.
  useEffect(() => {
    if (inHeader && expanded) inputRef.current?.focus();
  }, [inHeader, expanded]);
  ...
  const onInputKeyDown = (e) => {
    if (e.key === "ArrowDown") {
      ...
      // Escape closes the dropdown, then clears the query, and in the header
      // finally puts the field away.
      if (open) setOpen(false);
      else if (q) clear();
      else if (inHeader) setExpanded(false);
    }
  };
  ...
  return (
    <div ref={boxRef} data-testid="boat-search-box"
      className={inHeader ? className : `relative w-full max-w-xl ${className}`}>
      {inHeader ? (
        <Button variant="ghost" size="sm" aria-label="Search" aria-expanded={expanded}
          data-testid="site-search-toggle" onClick={() => setExpanded((v) => !v)}
      ...
      {(!inHeader || expanded) && (
        // In the header the panel hangs off the positioned header row rather
        // than the icon: a full-width bar on a phone, a right-aligned box
        // under the buttons on wider screens.
        <div className={inHeader
          ? "absolute left-0 right-0 top-[calc(100%+0.5rem)] z-50 mx-4 sm:left-auto sm:right-4 sm:mx-0 sm:w-[min(92vw,26rem)]"
          : undefined}>
