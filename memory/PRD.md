...
## Architecture
- **Backend**: FastAPI + MongoDB (motor). uuid string ids. Short-lived JWT access tokens tied to individual user accounts. Endpoints under `/api`.
...
- **Auth**: individual username + passcode accounts (bcrypt-hashed, per-club), with lockout + IP rate limiting. The webmaster account is bootstrapped once from the `WEBMASTER_PASSCODE` env var. Spectator = no login. (The legacy shared-PIN scheme was removed in the 2026-08 security hardening.)
- **Multi-club**: each club owns its classes/boats/series/races and user accounts. Officer & Admin access is hard-scoped server-side to the caller's own club — a `club_id`/`class_id`/`series_id` param can never widen access. The Webmaster account has no club and may manage clubs and enter any club's consoles.
...
- Webmaster — adds/removes/edits clubs, manages each club's logins, and has full Officer + Admin access to every club via per-club console launcher (`/webmaster`, then pick club).
...
- Full frontend: Landing (public, marquee, class/series tabs, standings, race folders), Login (username + passcode), Officer console (big finish buttons, notices, provisional editing, publish), Admin (Boats/Classes/Series/Historic tabs).
...
- **Rating divisions**: a class may field more than one rating system at once (IRC boats and YTC boats in the same class and the same series). `class.divisions = [{name, scoring_mode}, …]` (two or more; Admin class form) and `boat.division` (Admin boat form — blank means "by rating", placed by the certificate the boat holds, else the first division). One owner per rule: `_class_divisions` / `_boat_division` / `_division_fleet` on the server, `classDivisions` / `boatDivision` / `divisionTables` in `frontend/src/lib/helpers.js`. Races are re-sequenced within each division (`_resequence_race` → `_resequence_finished(boat_modes=…)`), and `/standings/series` + `/standings/overall` return `divisions: [payload per division]` alongside the whole-class table (which keeps per-boat history, exports and statistics working on one shape). The results pages, race page, PDF exports, regatta podiums and the officer console show the divisions as separate tables; a DNC is measured against the division's own fleet. The class and the series stay single entities.
...
- **Webmaster role**: the singleton webmaster user account (username `webmaster`), bootstrapped once from the `WEBMASTER_PASSCODE` env var. `require_webmaster` gates club CRUD and advert management. Webmaster passes `require_admin`/`require_officer` and bypasses `_ensure_club`, so it can run any club's consoles. (2026-08 hardening: per-user accounts, lockout, per-IP rate limiting, short-lived JWT with issuer/audience/token-version revocation, magic-byte upload validation, security headers; the shared-PIN scheme and plaintext club PIN fields were removed and migrated.)
...
- **Frontend**: Webmaster page (`/webmaster`) — club cards with add/edit/delete (name, colour) and per-club **Manage logins**, **Officer console** / **Admin console** launchers; Officer/Admin pages accept `?club=` (webmaster) and show a club picker when none is chosen.
